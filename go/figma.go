...
type FileResponse struct {
	...
	Document      DocumentClass             `json:"document"`     // The root node within the document
...
type ComponentValue struct {
	...
	BackgroundColor     BackgroundColorClass `json:"backgroundColor"`    // Background color of the node
	...
	Children            []ChildElement       `json:"children"`           // An array of nodes that are direct children of this node
...
//
// Color of the grid
type BackgroundColorClass struct {
...
type ChildElement struct {
	Children                []ChildElement        `json:"children"`                     // An array of canvases attached to the document; ; An array of top level layers on the canvas; ; An array of nodes that are direct children of this node; ; An array of nodes that are being boolean operated on
	ID                      string                `json:"id"`                           // a string uniquely identifying this node within the document
	Name                    string                `json:"name"`                         // the name given to the node by the user in the tool.; ; The name of the component
	Type                    NodeType              `json:"type"`                         // the type of the node, refer to table below for details
	Visible                 bool                  `json:"visible"`                      // whether or not the node is visible on the canvas
	BackgroundColor         *BackgroundColorClass `json:"backgroundColor,omitempty"`    // Background color of the canvas; ; Background color of the node
	...
	AbsoluteBoundingBox     *Rect                 `json:"absoluteBoundingBox,omitempty"`// Bounding box of the node in absolute space coordinates
	BlendMode               *BlendMode            `json:"blendMode,omitempty"`          // How this node blends with nodes behind it in the scene; (see blend mode section for more details)
	ClipsContent            *bool                 `json:"clipsContent,omitempty"`       // Does this node clip content outside of its bounds?
	Constraints             *LayoutConstraint     `json:"constraints,omitempty"`        // Horizontal and vertical layout constraints for node
	Effects                 []Effect              `json:"effects"`                      // An array of effects attached to this node; (see effects section for more details)
	IsMask                  *bool                 `json:"isMask,omitempty"`             // Does this node mask sibling nodes in front of it?
	LayoutGrids             []LayoutGrid          `json:"layoutGrids"`                  // An array of layout grids attached to this node (see layout grids section; for more details). GROUP nodes do not have this attribute
	Opacity                 *float64              `json:"opacity,omitempty"`            // Opacity of the node
	PreserveRatio           *bool                 `json:"preserveRatio,omitempty"`      // Keep height and width constrained to same ratio
	TransitionNodeID        *string               `json:"transitionNodeID"`             // Node ID of node to transition to in prototyping
	Fills                   []Paint               `json:"fills"`                        // An array of fill paints applied to the node
	StrokeAlign             *StrokeAlign          `json:"strokeAlign,omitempty"`        // Where stroke is drawn relative to the vector outline as a string enum; "INSIDE": draw stroke inside the shape boundary; "OUTSIDE": draw stroke outside the shape boundary; "CENTER": draw stroke centered along the shape boundary
	Strokes                 []Paint               `json:"strokes"`                      // An array of stroke paints applied to the node
	StrokeWeight            *float64              `json:"strokeWeight,omitempty"`       // The weight of strokes on the node
	CornerRadius            *float64              `json:"cornerRadius,omitempty"`       // Radius of each corner of the rectangle
	Characters              *string               `json:"characters,omitempty"`         // Text contained within text box
	...
	Style                   *TypeStyle            `json:"style,omitempty"`              // Style of text including font family and weight (see type style; section for more information)
	StyleOverrideTable      []TypeStyle           `json:"styleOverrideTable"`           // Map from ID to TypeStyle for looking up style overrides
	Description             *string               `json:"description,omitempty"`        // The description of the component as entered in the editor
	ComponentID             *string               `json:"componentId,omitempty"`        // ID of component that this instance came from, refers to components; table (see endpoints section below)
}
...
type Effect struct {
	BlendMode *BlendMode            `json:"blendMode,omitempty"`// Enum describing how layer blends with layers below; This type is a string enum with the following possible values
	Color     *BackgroundColorClass `json:"color,omitempty"`    // An RGBA color
	Offset    *Vector2              `json:"offset,omitempty"`   // A 2d vector
...
type Paint struct {
	Color                   *BackgroundColorClass `json:"color,omitempty"`        // Solid color of the paint
	...
	ScaleMode               *string               `json:"scaleMode,omitempty"`    // Image scaling mode
	Type                    FillType              `json:"type"`                   // Type of paint as a string enum
...
type ColorStop struct {
	Color    BackgroundColorClass `json:"color"`   // Color attached to corresponding position
...
type LayoutGrid struct {
	...
	Color       BackgroundColorClass `json:"color"`      // Color of the grid
...
type DocumentClass struct {
	Children []ChildElement `json:"children"`// An array of canvases attached to the document
	ID       string         `json:"id"`      // a string uniquely identifying this node within the document
...
type ClientMeta struct {
	X          *float64 `json:"x,omitempty"`          // X coordinate of the vector
	Y          *float64 `json:"y,omitempty"`          // Y coordinate of the vector
	NodeID     []string `json:"node_id"`              // Unique id specifying the frame.
	NodeOffset *Vector2 `json:"node_offset,omitempty"`// 2d vector offset within the frame.
}
...
const (
	Emoji FillType = "EMOJI"
	GradientAngular FillType = "GRADIENT_ANGULAR"
	GradientDiamond FillType = "GRADIENT_DIAMOND"
	GradientLinear FillType = "GRADIENT_LINEAR"
	GradientRadial FillType = "GRADIENT_RADIAL"
