...
//     ImageResponse data = Converter.ImageResponseFromJsonString(jsonString);
...
public class Converter {
    ...
    public static ImageResponse ImageResponseFromJsonString(String json) throws IOException {
        return getImageResponseObjectReader().readValue(json);
    }

    public static String ImageResponseToJsonString(ImageResponse obj) throws JsonProcessingException {
        return getImageResponseObjectWriter().writeValueAsString(obj);
    }
    ...
    private static ObjectReader ImageResponseReader;
    private static ObjectWriter ImageResponseWriter;

    private static void instantiateImageResponseMapper() {
        ObjectMapper mapper = new ObjectMapper();
        ImageResponseReader = mapper.reader(ImageResponse.class);
        ImageResponseWriter = mapper.writerFor(ImageResponse.class);
    }

    private static ObjectReader getImageResponseObjectReader() {
        if (ImageResponseReader == null) instantiateMapper();
        return ImageResponseReader;
    }

    private static ObjectWriter getImageResponseObjectWriter() {
        if (ImageResponseWriter == null) instantiateMapper();
